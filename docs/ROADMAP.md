...
> **状态：已完成。** 并行执行无新代码：`stream_chat` 已挂 rig `tool_concurrency(4)`（rig 0.41 以 `buffer_unordered` 并发派发，每次调用拿到独立 `ToolContext` 克隆，结果按调用序落历史，父子事件按 `internal_call_id` 配对——交错下不错配）。与原文的实现偏差：
> - UI 并行展示补的是**归属标识**而非新展示组件：`AgentEvent::Child` 增加 `label`（任务摘要截断），转换层给子代理的工具调用行 / 通知加 `[标识]` 前缀，交错事件可区分归属；标识只进展示字段，确认白名单仍按原始工具名匹配。
> - 验收测试在工具层做并发（`tokio::join!` 两路委派）：并发正确性与标识区分可确定性断言；真并行提速由 rig 自身 `tool_concurrency` 测试覆盖，不对 live API 做计时断言。
